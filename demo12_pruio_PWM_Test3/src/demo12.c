...
#define P_IN P9_42					//! The pin for CAP input.
#define P_OUTA P9_14				//! The pin for PWM output.
#define P_OUTB P9_16
...
{   float_t f1 = 500; 		//      Variable for calculated frequency.
    ...
    if (pruio_pwm_setValue(Io, P_OUTA, f0, d0))
    {   printf("failed setting output @P_OUTA (%s)\n", Io->Errr); break;
    }
    if (pruio_pwm_setValue(Io, P_OUTB, f0, d0))
    {   printf("failed setting output @P_OUTB (%s)\n", Io->Errr); break;
    }
    //           pin config OK, transfer local settings to PRU and start
    if (pruio_config(Io, 1, 0x1FE, 0, 0))
...
//    msg= pruio_rb_start(Io);
    ...
    {	if (1 == isleep(1)) 				// got a char
    	{	switch (getchar())					// evaluate keystroke
		   ...
		   if (pruio_pwm_setValue(Io, P_OUTA, f0, d0))	//   set new output
		   {   printf("Afailed setting PWM output (%s)\n", Io->Errr); break;
		   }
		   if (pruio_pwm_setValue(Io, P_OUTB, f0, d0))	//   set new output
		   {   printf("Bfailed setting PWM output (%s)\n", Io->Errr); break;
		   }
      ...
      printf("\r    CapFreq: %10f , CapDuty: %10f     ", f1, d1); // info
      fflush(STDIN_FILENO);
    } //end while
...
finish:
	...
	if (pruio_pwm_setValue(Io, P_OUTA, 0, 1))	   //   set new output
