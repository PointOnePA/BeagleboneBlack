 ...
 *
 * This is a 1/2-D table lookup facility originally written by Mark
 * Schimmelman in 2004 and later modified by Justin Dobbs in 2005.  Each routine
 * looks up data in a table structure, interpolating as needed between data
 * points.  The 2-D version looks up along 2 axes and interpolates in two
 * dimensions.
 *
 * I have removed the table title and saved index fields.  The saved index field
 * allowed a lookup to start at the coordinates where the last lookup finished,
 * thus saving from having to iterate from the beginning of the table each time.
 * However, that feature is useful only when a table is used exactly once with
 * a slowly changing input value, and in practice tables tend to be used more
 * than once (such as for looking up temperatures from a thermistor ADC
 * reading).
 *
 * The coding style here was atrocious, and I couldn't stand it anymore, so I
 * rewrote and tested both routines.  They are functionally equivalent to the
 * original versions.
 *
 * <h2>Limitations</h2>
 * - The table axes (input values) must monotonically increase, or the lookup
 *   will fail.
 * - The index data type is <code>unsigned char</code>.  On most platforms,
 *   this means that a table cannot be longer than 256 elements.
 *
 * @author Justin Dobbs
 * @date 2005-2011
 */
